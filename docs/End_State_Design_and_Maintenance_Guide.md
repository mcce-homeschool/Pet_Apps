...
- **incomeView** (derived, not a repo/table): `getIncomeRows({includeArchived})` and
  ...
  per litter. Stores nothing; recomputed on every load. Also exports
  `incomeLineItems(sourceType, record)` — the cash line items (drops the non-cash
  `pick`, tags each with its label) that the invoice/receipt generator and print
  page both build from, so a document can't show a component the ledger wouldn't.
  See §21 and §24.
...
Financials print docs: `invoice` (print-only invoice/receipt generator, §24 — not a
nav entry, reached from the Financials hub's "Invoice / Receipt" generator modal).
...
The header also renders the resolving own-kennel's `logo_data_url` (§24) above the
kennel name when one is set.
